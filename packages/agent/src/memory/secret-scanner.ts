 ...
 *
 * 1:1 replicate of claude-code-source/src/services/teamMemorySync/secretScanner.ts.
 */
...
const SECRET_RULES: SecretRule[] = [
  ...
  },
  {
    ...
    // gitleaks: SG\.(?i)[a-z0-9=_\-\.]{66} -> JS: case-insensitive via char class
...
export function redactSecrets(content: string): string {
  ...
  for (const re of redactRules) {
    ...
    content = content.replace(re, (_match, g1) =>
      typeof g1 === "string" ? _match.replace(g1, "[REDACTED]") : "[REDACTED]",
    );
  }
  return content;
}
